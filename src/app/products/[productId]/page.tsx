  ...
  return (
    <div className="mx-auto mb-10 mt-20 grid max-w-[97%] items-center gap-10 sm:max-w-[90%] sm:grid-cols-[1fr_2fr]">
      <div className="space-y-3">
        {isLoading ? (
          <ImageLoader height="60" svgHeight="10" />
        ) : (
        ...
        <div className="items-center flex justify-center">
          <div className="grid w-[98%] gap-3 grid-cols-[repeat(auto-fit,_minmax(120px,_1fr))] ">
            {data.map((laptop, index) => (
              <div
                onMouseEnter={() => handleMouseEnter(laptop, index)}
                className={
                  activeContainer === index
                    ? `flex justify-center rounded-md h-14  hover:bg-gradient-to-r hover:from-emerald-600 hover:to-emerald-800`
                    : `flex justify-center rounded-md h-14  hover:bg-gradient-to-r hover:from-emerald-600 hover:to-emerald-800`
                }
                key={Math.random()}
              >
                {isLoading ? (
                  <ImageLoader height="14" svgHeight="5" />
                ) : (
                  <Image
                    src={laptop}
                    alt="gaming laptop"
